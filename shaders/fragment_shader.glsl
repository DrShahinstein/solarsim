...
vec3 ray_march(vec3 ro, vec3 rd) {
    ...
    for (int i = 0; i < MAX_STEPS; i++) {
        ...
        if (dist < SURF_DIST || total_dist > MAX_DIST) break;
    }
    ...
    if (body_index < 0) return vec3(1.0, 0.0, 1.0);
    
    vec3 color = bodies[body_index].color;
    float diff = max(dot(normal, light_dir), 0.1);
    vec3 reflect_dir = reflect(-light_dir, normal);
    float spec = pow(max(dot(rd, reflect_dir), 0.0), 32.0);
    
    return color * (diff + 0.3) + vec3(0.8) * spec;
}
